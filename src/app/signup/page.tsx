...
export default function SignupPage() {
  ...
  const onProfileSubmit = async (data: ProfileFormValues) => {
    ...
    try {
      ...
      router.push(`/verify?email=${encodeURIComponent(tempCredentials.email)}${result.data?.emailError ? '&emailError=true' : ''}${next ? `&next=${encodeURIComponent(next)}` : ''}`);
    } catch (err: any) {
      ...
      <div className="w-full max-w-md transform scale-[1.1] origin-center transition-transform">
            ...
            <>
              ...
              <div className="px-6 pb-6 space-y-4">
                ...
                <div className="text-sm text-center text-muted-foreground">
                  ...
                  <Link href={`/login${next ? `?next=${encodeURIComponent(next)}` : ""}`} className="text-[#a9d6f3] font-bold hover:underline underline-offset-4">
                    Login
                  </Link>
                </div>
              </div>
        ...
        <p className="mt-8 px-8 text-center text-xs text-muted-foreground">
          ...
          <Link
            ...
            className="underline underline-offset-4 hover:text-[#a9d6f3]"
          >
            Terms of Service
          </Link>{" "}
          and{" "}
